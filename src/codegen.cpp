...
#ifdef LLVM37
DIType *julia_type_to_di(jl_value_t *jt, DIBuilder *dbuilder, bool isboxed = false)
#else
DIType julia_type_to_di(jl_value_t *jt, DIBuilder *dbuilder, bool isboxed = false)
#endif
{
...
// Functions
//    jl_cgval_t emit_expr(jl_value_t *expr, jl_codectx_t *ctx);
//
//    Value *emit_local_root(jl_codectx_t *ctx, jl_varinfo_t *vi = NULL);
//    void mark_gc_use(const jl_cgval_t &v);
//    Value *make_jlcall(ArrayRef<const jl_cgval_t*> args, jl_codectx_t *ctx);
//    Value *global_binding_pointer(jl_module_t *m, jl_sym_t *s,
//                                         jl_binding_t **pbnd, bool assign, jl_codectx_t *ctx);
//    jl_cgval_t emit_checked_var(Value *bp, jl_sym_t *name, jl_codectx_t *ctx, bool isvol, MDNode *tbaa);
//    Value *emit_condition(jl_value_t *cond, const std::string &msg, jl_codectx_t *ctx);
//    GlobalVariable *prepare_global(GlobalVariable *G, Module *M = jl_builderModule);
//    llvm::Value *prepare_call(llvm::Value *Callee);
